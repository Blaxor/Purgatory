...
public class ConfigManager {
	...
	private void setMessages() {
		...
		if(!plugin.isBungeeEnabled()) {
			getMessages().options().header("Purgatory messages config \n For a new line use %newline% ");
			...
			getMessages().addDefault("TempBan.Format",
					"&4You have been banned by %admin% %newline% because %reason% %newline% for %time%s");
			...
			getMessages().addDefault("TempBan.broadcast",
					"&7[&aPurgatory&7]&a%player% have been tempbanned by %admin% for %time%s!");
			...
			getMessages().addDefault("Warning.Receive",
					"&4You have been warned(%warning%/%warning_max%) by %admin% because : %reason%");
			getMessages().addDefault("Warning.Send", "&4You warned %player% because : %reason%");
			getMessages().addDefault("Warning.broadcast",
					"&7[&aPurgatory&7]%admin% warned %player% for the reason : %reason%");
			getMessages().addDefault("Warning.DefaultReason", "This is a default reason for warnings");
			getMessages().addDefault("Info.Format",
					"&4%player%'s Info%newline%&4Is banned: %isban% %newline%&4Time: %time% %newline%&4Reason: %reason% %newline%&4Warnings: %warnings%");
			getMessages().addDefault("Yes", "Yes");
			getMessages().addDefault("No", "No");
			getMessages().addDefault("BanChat.Prefix", "[&cBanChat&f]&e%player% &e-&f ");
			...
			getMessages().addDefault("Protected", "&4In this place you can`t use the trolls!");
			...
			getMessages().addDefault("Troll.SmokeScreen.Description",
					"&7Spawns an orb of grayish particles %newline%&7around the cheater that lasts\r\n 5 seconds. %newline% %newline% &7Cooldown is : &4&b%cooldown%");
			getMessages().addDefault("Troll.Burn.Title", "Burn");
			getMessages().addDefault("Troll.Burn.Description",
					"&7Lights the block %newline%&7where the cheater %newline%&7is standing on for fire for 1 second %newline% %newline% &7Cooldown is : &4&b%cooldown%");
			getMessages().addDefault("Troll.Flip.Title", "Flip");
			getMessages().addDefault("Troll.Flip.Description",
					"&7Flipst the cheater around 180 degrees%newline% %newline% &7Cooldown is : &4&b%cooldown%");
			getMessages().addDefault("Troll.Creeper.Title", "Creeper");
			getMessages().addDefault("Troll.Creeper.Description",
					"&7Play the sound of a creeper about %newline%&7to explode behind the cheater%newline% %newline% &7Cooldown is : &4&b%cooldown%");
			getMessages().addDefault("Troll.Web.Title", "Web");
			getMessages().addDefault("Troll.Web.Description",
					"&7Spawns a web block on whatever %newline%&7space the cheater's legs are curently on %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Lag.Title", "Lag");
			getMessages().addDefault("Troll.Lag.Description",
					"&7Over the course of 3 seconds%newline%&7teleport the cheater to themself %newline%&7every half of second making them 'lag' %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Mole.Title", "Mole");
			getMessages().addDefault("Troll.Mole.Description",
					"&7Buries the cheater 3 blocks under ground %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Infection.Title", "Infection");
			getMessages().addDefault("Troll.Infection.Description",
					"&7Buries the cheater 3 blocks under ground%newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Storm.Title", "Storm");
			getMessages().addDefault("Troll.Storm.Description",
					"&7Launchest the player in a random direction%newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Miner.Title", "Miner");
			getMessages().addDefault("Troll.Miner.Description",
					"&7Plays sound of a serier of blocks %newline%&7breaking near the cheater.%newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Paralysis.Title", "Paralysis");
			getMessages().addDefault("Troll.Paralysis.Description",
					"&7Gives the player mining fatigue %newline%&7for 10 seconds.%newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Pumpkin.Title", "Pumpkin");
			getMessages().addDefault("Troll.Pumpkin.Description",
					"&7Replace cheater's head with pumpking %newline%&7and old head is place it in inventory%newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Bouncy.Title", "Bouncy");
			getMessages().addDefault("Troll.Bouncy.Description",
					"&7Knock the cheater 25 blocks in air %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Slow.Title", "Slow");
			getMessages().addDefault("Troll.Slow.Description",
					"&7Gives the cheater slowness 4 for 5 seconds. %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.JumpBoost.Title", "JumpBoost");
			getMessages().addDefault("Troll.JumpBoost.Description",
					"&7Give the cheater jump boost 10 for 10 seconds %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.Speed.Title", "Speed");
			getMessages().addDefault("Troll.Speed.Description",
					"&7Gives the cheater speed 100 for 5 seconds %newline% %newline% &7Cooldown is : &4&b%cooldown% ");
			getMessages().addDefault("Troll.MobSquad.Title", "MobSquad");
			getMessages().addDefault("Troll.MobSquad.Description",
					"&7Spawns a gang of randomly %newline%&7chosen mobs around the cheater %newline% %newline% &7Cooldown is : &4&b%cooldown%");
